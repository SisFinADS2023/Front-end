...
const InputSenha = ({placeholder}) => {
    const [isVisible, setIsVisible, inputValue, setInputValue] = useState(false)
        ...
        <>
            <div className="relative">
                <input id="password-login" className={`mb-6 border-solid border-2 border-gray-900 border-opacity-10 text-[#A2A3A4] px-2 font-normal text-base h-[48px] rounded w-[100%]`}
                    type={isVisible ? "text" : "password"} name='Senha' value={inputValue} placeholder={placeholder} onChange={handleInputChange}></input>
                {
                    isVisible
